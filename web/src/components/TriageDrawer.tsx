...
const TriageDrawer: React.FC<TriageDrawerProps> = ({ isOpen, onClose, alert, onActionCompleted }) => {
  ...
  // Force cleanup of any existing polling connections
  ...
  // Handle ESC key (replace malformed previous block)
  useEffect(() => {
  ...
  const startEventPolling = useCallback((runId: string) => {
    // Only start polling if this is still the current run ID
    if (currentRunIdRef.current !== runId) {
      console.log('Skipping polling for stale run ID:', runId, 'current:', currentRunIdRef.current);
      return;
    }
    ...
    const pollForResults = async () => {
      ...
      if (currentRunIdRef.current !== runId || streamCompletedRef.current || !isOpen) {
      ...
      try {
        // Poll the triage result endpoint
        ...
        if (!response.ok) {
          if (response.status === 404) {
            // Triage not found yet, continue polling
        ...
        // Check if we have a decision/recommendation (handle nested structure)
        const decision = result.decision || result;
        if (decision && decision.recommended_action && decision.risk_score !== undefined) {
          console.log('Found completed triage result:', decision);
          
          // Create a decision_finalized event
          const finalEvent = {
        ...
        if (reconnectAttemptsRef.current < 5 && !streamCompletedRef.current) {
          ...
          console.log(`Retrying poll in ${delay}ms (attempt ${reconnectAttemptsRef.current})`);
  ...
  const executeAction = async (action: string) => {
    ...
    try {
      ...
      if (action === 'freeze_card') {
        ...
        await markFalsePositive.mutateAsync({ alertId: alert.id, reason: 'Manually reviewed and determined false positive' });
        response = undefined; // Treat as success below
      }

      if ((response && response.ok) || action === 'mark_false_positive') {
        const result = action === 'mark_false_positive' ? { status: 'FALSE_POSITIVE' } : await response!.json();
        ...
        if (result.status === 'PENDING_OTP') {
          ...
          // If marking as false positive, close the drawer since the alert will be removed from the list
