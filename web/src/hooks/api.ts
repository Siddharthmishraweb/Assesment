...
export const useAlertCounts = () => {
  return useQuery({
    ...
    queryFn: () => apiClient.get<{ total: number; open: number }>('/alerts/count'),
...
export const useCustomerInsightsSummary = (customerId: string, lastDays: number = 90) => {
  return useQuery({
...
export const useEvaluationResults = (evaluationId: number) => {
  return useQuery({
    ...
    queryFn: () => apiClient.get<EvaluationResults>(`/api/v1/evaluations/${evaluationId}/results`),
    enabled: !!evaluationId,
