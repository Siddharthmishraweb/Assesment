...
export const triageRoutes = (config: RouteConfig): Router => {
  ...
  router.get('/:runId/stream', async (req: Request, res: Response): Promise<void> => {
    ...
    const sendEvents = async () => {
      ...
      try {
        ...
        if (hasDecision) {
          ...
          // Clear any pending poll interval immediately
          if (pollInterval) {
    ...
    const scheduleNext = () => {
      // Multiple checks to prevent race conditions
      if (isCompleted || res.writableEnded || res.destroyed) {
        logger.info('Stream completed or ended, not scheduling next poll', { runId, isCompleted, writableEnded: res.writableEnded, destroyed: res.destroyed });
        return; // Stop scheduling if completed or response ended
      }
      
      pollInterval = setTimeout(async () => {
        // Double-check before executing to prevent race conditions
        if (isCompleted || res.writableEnded || res.destroyed) {
          logger.info('Skipping poll due to completed stream or closed response', { runId });
          return;
        }
        ...
        // Only schedule next if still not completed and response is still open
        if (!isCompleted && !res.writableEnded && !res.destroyed) {
          scheduleNext();
        }
      }, currentInterval);
