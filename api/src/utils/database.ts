...
export class Database {
  ...
  private async ensureEvaluationTables(): Promise<void> {
    try {
      ...
      const evalTableCheck = await this.query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'evaluations');");
      ...
      if (!evalTableCheck[0].exists) {
        ...
        const fs = require('fs');
        const path = require('path');
        const evaluationMigrationPath = path.join(__dirname, '../../migrations/add_evaluations.sql');
