...
Provide internal support agents a unified console to load customer activity, generate AI-assisted insights, and execute safe, explainable multi-agent triage actions with full observability, determinism, and offline capability.
...
**Success Metrics (Indicative SLO Targets):**
...
```mermaid
flowchart LR
   subgraph AgentConsole[Support Agent Browser]
      UI[React + Vite SPA]
   end
   UI --> API[(Express API)]
   API --> PG[(PostgreSQL)]
   API --> REDIS[(Redis Cache / Rate Limit / Circuit State)]
   API --> SSE[(SSE Stream)]
...
sequenceDiagram
   ...
   Agent->>UI: Open Alert & Click "Open Triage"
   UI->>API: POST /api/triage (alertId)
   API->>ORCH: Start workflow (plan build)
   ORCH->>REDIS: Read circuit states / rate limits
   ORCH->>PG: Fetch customer & transaction context
   ORCH->>FRAUD: Execute fraud analysis (timeout 1s)
   ORCH->>INSIGHTS: Derive spend patterns
   ORCH->>KB: Retrieve policy citations
   ORCH->>COMPLIANCE: Validate action eligibility
   ORCH->>REDACT: Redact sensitive fields
   ORCH->>SUMMARY: Synthesize recommendation
   ORCH-->>API: Stream events (tool_update, fallback_triggered)
   API-->>UI: SSE events to Drawer
   UI->>API: POST /api/action/freeze-card (if accepted)
   API->>PG: Persist case + event
   API->>METRICS: Increment counters
   API-->>UI: Action result (FROZEN)
```
...
| Scenario | Fixture / Eval | Endpoint(s) | Metrics/Logs Observed | Fallback Usage |
|----------|----------------|-------------|------------------------|----------------|
| 1. Freeze w/ OTP | `eval-001-freeze-otp.json` | POST /api/triage, POST /api/action/freeze-card | `tool_call_total{tool=fraud}`, `action_blocked_total{policy=otp_required}` | Expected only if risk tool failure |
| 2. Dispute creation | `eval-002-dispute-creation.json` | POST /api/triage, POST /api/action/open-dispute | `tool_call_total{tool=insights}`, case event log | None |
| 3. Duplicate pending vs captured | `eval-003-duplicate-analysis.json` | POST /api/triage | KB query logs, fraud agent reasoning | Possible if insights timeout |
| 4. Risk tool timeout → fallback | `eval-004-tool-timeout.json` | POST /api/triage | `agent_fallback_total{tool=fraud}`, trace `fallback_triggered` | Yes (fraud) |
| 5. 429 behavior | `eval-005-rate-limiting.json` | POST /api/triage | `rate_limit_block_total` increments | N/A |
| 6. PII redaction | `eval-006-pii-redaction.json` | POST /api/triage | Logs include `masked=true` | As needed |
| 7. Performance (p95 latency) | `eval-007-performance.json` | GET /api/customer/:id/transactions | `api_request_latency_ms` histogram | None |

## Glossary

| Term | Definition |
|------|------------|
| Triage | Automated multi-agent evaluation of an alert culminating in a recommended action. |
| Fallback | Deterministic substitute result when an agent times out or breaker is open. |
| Circuit Breaker | Mechanism preventing repeated failing calls to a tool for a cool-down period. |
| Keyset Pagination | Pagination technique using inequality on last seen ordered tuple to fetch next slice efficiently. |
| PAN | Primary Account Number; card number subject to redaction patterns. |
| SSE | Server-Sent Events; uni-directional streaming from server to client over HTTP. |
| Eval | Predefined scenario JSON used to validate correctness & resilience. |

## Open Questions / Future ADR Candidates
1. Should we introduce message queue (e.g. NATS or Kafka) for triage event decoupling?  
2. Do we require adaptive risk scoring models (ML) or retain deterministic heuristics?  
3. Introduce multi-region read replicas for disaster recovery?  
4. Expand RBAC with fine-grained policy engine (OPA) vs current role hints?  

## Change Log
- 2025-11-16: Added mission statement, context & sequence diagrams, acceptance scenario mapping, glossary, open questions section; wording refinements.
