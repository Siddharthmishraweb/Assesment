...
9. [Redis Key Naming Conventions](#redis-key-naming-conventions)
10. [Evaluation Performance Analytics](#evaluation-performance-analytics)
...
### Core Tables Structure (Current Implementation Alignment)
...
CREATE INDEX CONCURRENTLY idx_transactions_customer_ts 
  ON transactions (customer_id, ts DESC);
...
-- Future risk scoring index placeholder
-- CREATE INDEX CONCURRENTLY idx_transactions_risk_score ON transactions (risk_score DESC, ts DESC) WHERE risk_score IS NOT NULL;
...
```typescript
// Transactions API (keyset pagination expected)
...
### Sequence Diagram (Triage Workflow)
```mermaid
sequenceDiagram
  participant API
  participant Planner
  ...
  API->>Planner: Build plan
  Planner->>Fraud: riskSignals()
  Fraud-->>Planner: riskScore + reasons | timeout→fallback
  Planner->>Insights: recentTx() & patterns
  Insights-->>Planner: categories + merchant concentration
  Planner->>KB: kbLookup()
  KB-->>Planner: citations[]
  Planner->>Compliance: policyGate()
  Compliance-->>Planner: status / deny reason
  Planner->>Redactor: scrub sensitive
  Redactor-->>Planner: sanitized trace
  Planner->>Summarizer: proposeAction()
  Summarizer-->>Planner: recommended action
  Planner-->>API: Stream SSE events per step
```
...
| Purpose | Pattern | TTL | Notes |
|---------|---------|-----|-------|
| Rate limit bucket | `rl:{ip}` | window | Stores token counts / reset time |
| Circuit breaker state | `cb:{tool}` | 30s | Tracks consecutive failures & open state |
| Triage run trace partial | `triage:run:{runId}:step:{n}` | 5m | Ephemeral step details before persistence |
| Idempotency keys | `idem:{action}:{key}` | 24h | Stores prior action response |
| Cached insights summary | `cache:insights:{customerId}` | 10m | Avoid recomputation of static patterns |

## Evaluation Performance Analytics

Endpoint: `GET /api/v1/evaluations/performance?days=30`

Response Shape:
```json
{
  "performanceOverTime": [
    {"date": "2025-11-01", "avgScore": 0.82, "minScore": 0.71, "maxScore": 0.93, "count": 12}
  ],
  "categoryBreakdown": [
    {"category": "fraud", "avgScore": 0.75, "count": 40},
    {"category": "compliance", "avgScore": 0.88, "count": 25}
  ],
  "recentTrends": [
    {"id": "uuid", "name": "eval-007", "score": 0.84, "status": "COMPLETED", "updated_at": "2025-11-16T12:33:00Z"}
  ],
  "summary": {"daysAnalyzed": 30, "totalDataPoints": 220}
}
```
...
Used by: `PerformanceChart` component (full & compact modes).

## Glossary

| Term | Definition |
|------|------------|
| Evaluation | A batch test run verifying scenario correctness and resilience. |
| Fallback | Deterministic agent response when timeout or circuit open occurs. |
| Circuit Breaker | Failure isolation pattern preventing repeated tool calls. |
| Keyset Cursor | Base64 encoded JSON containing sort field + id for pagination. |
| PAN | Primary Account Number requiring redaction. |
| SSE | Server-Sent Events channel streaming orchestration events. |

## Change Log
- 2025-11-16: Updated schemas to UUID, added evaluations tables, added sequence diagram, Redis key naming, performance analytics section, glossary; polished capitalization & formatting.
